# app.py
"""
InvyPro — Single-file Streamlit Inventory Manager with Cybersecurity Enhancements
...
- Minimal external deps: streamlit, pandas, altair, pytz, sqlcipher3, pycryptodome
"""
...
# App config
# ---------------------------
st.set_page_config(page_title="InvyPro — Secure Inventory Manager", page_icon="🔒", layout="wide")

# ---------------------------
# Security Configuration
# ---------------------------
# Get encryption key from environment variable or use a default (for demo only)
DB_ENCRYPTION_KEY = os.getenv("DB_ENCRYPTION_KEY", "default_demo_key_change_in_production_12345")
BACKUP_ENCRYPTION_KEY = os.getenv("BACKUP_ENCRYPTION_KEY", "backup_demo_key_change_in_production_67890")
...
st.markdown(
    ...
    .security-banner{background-color:#d1ecf1;color:#0c5460;padding:10px;border-radius:5px;margin-bottom:15px;}
    </style>
    """,
...
# ---------------------------
# Database utilities with SQLCipher encryption
# ---------------------------
...
def get_conn():
    ...
    if _conn is None:
        try:
            ...
            _conn.execute(f"PRAGMA key='{DB_ENCRYPTION_KEY}';")
            _conn.execute("PRAGMA cipher_compatibility = 3;")
        except ImportError:
            # Fall back to standard SQLite if SQLCipher is not available
            st.warning("SQLCipher not available. Using standard SQLite (no encryption).")
            _conn = sqlite3.connect(DB_PATH, check_same_thread=False)
...
# ---------------------------
def encrypt_data(data: bytes, key: str) -> Tuple[bytes, bytes]:
    ...
    return salt + cipher.nonce + tag + ciphertext, derived_key
...
def signup(username: str, email: str, password: str, organization: str):
    ...
    try:
        with db_session() as conn:
            conn.execute(
                "INSERT INTO users (username, email, password_hash, salt, organization, role, is_active, created_at) VALUES (?,?,?,?,?,'viewer',1,?);",
...
def require_role(required_role: str):
    ...
    if st.session_state.role == "admin":
        return True
    if required_role == "viewer" and st.session_state.role in ["viewer", "manager", "admin"]:
        return True
    if required_role == "manager" and st.session_state.role in ["manager", "admin"]:
        return True
    if required_role == "admin" and st.session_state.role == "admin":
        return True
    return False

def role_based_warning():
    st.warning(f"Insufficient permissions. Your role: {st.session_state.role}")
...
# Security banner
st.sidebar.markdown('<div class="security-banner">🔒 Database Encrypted</div>', unsafe_allow_html=True)
...
# navigation
PAGE_OPTIONS = ["Dashboard", "Products", "Sales & Restock", "Suppliers", "Transactions", "Stock History", "Bulk Upload / Export", "Backup & Restore", "Settings"]
...
if page == "Dashboard":
    ...
    if st.session_state.demo_mode:
        st.markdown('<div class="hero"><h1>InvyPro — Secure Inventory Manager</h1><p class="muted">Sign up or log in to manage your inventory. Below is a demo preview.</p></div>', unsafe_allow_html=True)
    ...
    if not st.session_state.demo_mode:
        try:
            import sqlcipher3
            st.success("🔒 Database encryption: Enabled (SQLCipher)")
        except ImportError:
            st.warning("⚠️ Database encryption: Not available (using standard SQLite)")
...
# ---------------------------
    ...
    if st.session_state.demo_mode:
        ...
        if not require_role("viewer"):
            role_based_warning()
            st.stop()
    
    # Product management functions
    def upsert_product_form(sku, name, category, supplier_name, cost_price, sell_price, qty, reorder_level):
    ...
    if st.session_state.demo_mode:
        ...
        if products.empty:
            ...
            # Delete product option (only for admins and managers)
            if require_role("manager"):
                ...
                st.info("🔒 Delete permission requires manager role")
